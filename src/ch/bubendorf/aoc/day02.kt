...
fun main(args : Array<String>) {
    ...
    val sum1 = input.split("\n")
            .map { minmaxdiff(it) }
            .sum()

    println("Summe 1 = $sum1")

    val sum2 = input.split("\n")
            .map { division(it) }
            .sum()

    println("Summe 2 = $sum2")

}

fun minmaxdiff(text:String) : Int {
    val summary = text.split("\\s+".toRegex())
            .map { it.toInt() }
            .summarizingInt()
    return summary.max - summary.min
}

fun division(text:String) : Int {
    val numbers = text.split("\\s+".toRegex()).map{it.toInt()}
...
fun Collection<Int>.summarizingInt(): SummaryStatisticsInt
        = this.fold(SummaryStatisticsInt()) {
    stats, num -> stats.accumulate(num)
}
